  ...
  return (
    <div className="flex-1 flex flex-col max-w-5xl mx-auto w-full px-4 py-6">
      ...
      <div className="text-center mb-8">
        <h1 className="text-2xl font-bold text-[var(--color-studio-fg)] mb-2">
          AI 演播厅
        </h1>
        ...
        {(['live', 'ended'] as const).map((tab) => (
          <button
            ...
            className={`px-4 py-1.5 text-xs font-medium rounded-md transition-all duration-150 cursor-pointer
              ${activeTab === tab
                ? 'bg-[var(--color-studio-elevated)] text-[var(--color-studio-fg)] shadow-sm'
      ...
      {listLoading ? (
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <Button variant="secondary" size="sm" onClick={() => fetchList(activeTab)}>
              重试
            </Button>
          </div>
        </div>
